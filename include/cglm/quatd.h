...
/*
 Macros:
   GLM_QUATD_IDENTITY_INIT
   GLM_QUATD_IDENTITY

 Functions:
   CGLM_INLINE void glm_quatd_identity(versord q);
   CGLM_INLINE void glm_quatd_init(versord q, double x, double y, double z, double w);
   CGLM_INLINE float glm_quatd_norm(versord q);
   CGLM_INLINE void glm_quatd_conjugate(versord q, versord dest);
   CGLM_INLINE void glm_quatd_inv(versord q, versord dest);
   CGLM_INLINE void glm_quatd_mul(versord p, versord q, versord dest);
   CGLM_INLINE void glm_quatd_mat4(versord q, mat44 dest);
...
#define GLM_QUATD_IDENTITY_INIT  {0.0, 0.0, 0.0, 1.0}
#define GLM_QUATD_IDENTITY       ((versord)GLM_QUATD_IDENTITY_INIT)
...
CGLM_INLINE
void
glm_quatd_identity(versord q) {
  CGLM_ALIGN(16) versord v = GLM_QUATD_IDENTITY_INIT;
  glm_vec4d_copy(v, q);
}
...
CGLM_INLINE
void
glm_quatd_init(versord q, double x, double y, double z, double w) {
  q[0] = x;
  q[1] = y;
  q[2] = z;
  q[3] = w;
...
CGLM_INLINE
double
glm_quatd_norm(versord q) {
  return glm_vec4d_norm(q);
}
...
CGLM_INLINE
void
glm_quatd_conjugate(versord q, versord dest) {
  glm_vec4d_negate_to(q, dest);
  dest[3] = -dest[3];
}
...
CGLM_INLINE
void
glm_quatd_inv(versord q, versord dest) {
  CGLM_ALIGN(16) versord conj;
  glm_quatd_conjugate(q, conj);
  glm_vec4d_scale(conj, 1.0f / glm_vec4d_norm2(q), dest);
}
...
CGLM_INLINE
void
glm_quatd_mul(versord p, versord q, versord dest) {
  /*
...
/*!
 * @brief convert quaternion to mat4d
 *
 * @param[in]   q     quaternion
...
CGLM_INLINE
void
glm_quatd_mat4d(versord q, mat4d dest) {
  double w, x, y, z,
        xx, yy, zz,
        xy, yz, xz,
        wx, wy, wz, norm, s;

  norm = glm_quatd_norm(q);
...
#if 0 /* Unimplemented - unconverted functions*/
/*!
